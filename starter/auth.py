...
AUTH0_DOMAIN = 'charlieagency.us.auth0.com' 
ALGORITHMS = ['RS256']
API_AUDIENCE = 'jynn'
...
'''
Checks if a permission is sufficient
    @INPUTS
...
def check_permissions(permission, payload):
    if 'permissions' not in payload:
        raise AuthError({
            'code': 'invalid_claim',
            'description': 'Permissions do not exist in jwt.'
        }, 401)
    if permission not in payload['permissions']:
        raise AuthError({
            'code': 'unauthorized',
            'description': 'Client has no permission.'
        }, 403)
